  ...
  const handleSendEmail = async () => {
    ...
    try {
      const res = await fetch('/api/send-email', {
        ...
        body: JSON.stringify({ email: email, bookType: 'hack' })
      });
      if (res.ok) {
        setEmailSent(true);
      } else {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white p-6 font-sans relative">
      ...
      <div className="max-w-5xl mx-auto text-center pt-20">
        <div className="inline-block px-4 py-1.5 bg-[#00ff41]/10 border border-[#00ff41]/30 rounded-full text-[#00ff41] text-xs font-bold uppercase tracking-widest mb-6">
        ...
        <p className="text-gray-500 text-sm mb-10">Your encrypted digital edition is ready.</p>
        
        <a href="/hacking.pdf" target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center w-full max-w-sm mx-auto mb-6 bg-gradient-to-r from-[#00ff41] to-[#008f11] text-black font-black py-4 px-8 rounded-xl hover:scale-105 transition-transform">
          <i className="fa-solid fa-download mr-3 text-xl"></i> Access Blueprint
        </a>

        {/* YAHAN HAI WO EMAIL BOX 👇 */}
        <div className="max-w-sm mx-auto mb-10 bg-zinc-900/50 p-5 rounded-xl border border-[#00ff41]/20 shadow-lg text-left">
          <h3 className="text-sm font-bold mb-3 text-white"><i className="fa-solid fa-envelope text-[#00ff41] mr-2"></i> Get a Backup Link on Email:</h3>
          {!emailSent ? (
            <div className="flex flex-col gap-3">
              <input type="email" placeholder="Enter your email address" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-black/60 border border-white/20 rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-[#00ff41] text-white transition-colors" />
            ...
            <div className="bg-[#00ff41]/10 border border-[#00ff41]/30 p-3 rounded-lg text-center">
              <p className="text-sm text-[#00ff41] font-bold"><i className="fa-solid fa-check-circle mr-1"></i> Email Sent!</p>
        ...
        <div className="hidden md:block w-full max-w-4xl mx-auto bg-zinc-900/30 border border-[#00ff41]/10 rounded-2xl overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.8)]">
          <iframe src="/hacking.pdf" className="w-full h-[75vh]" title="Ethical Hacking Blueprint" />
        </div>
      </div>
    </div>
  );
